...
To the best of the authors' knowledge, existing studies do not provide a compression framework for Lyapunov-certified neural network controllers that explicitly preserves stability guarantees for singularly perturbed systems. This paper addresses that gap for singularly perturbed nonlinear systems by decomposing the control-sensitive Fisher information matrix (CS-FIM) asymptotically along the slow manifold and boundary layer, establishing that the dominant parameter-space curvature is determined to leading order in ε by the slow subsystem alone. The occupation-measure derivation is made rigorous through averaging theory and Tikhonov asymptotics, yielding the precise splitting with O(ε²) remainder in total variation. A Davis–Kahan bound controls the resulting subspace error by O(ε/γ_r), motivating a Stiefel-manifold projection that preserves the parameter directions critical for slow-manifold control sensitivity. Regional input-to-state stability (ISS) of the compressed closed-loop system is proved via a composite Lyapunov function for all ε ∈ (0, ε*), and reformulated as semi-global practical ISS to align with the broader nonlinear control literature.

This repository contains the MATLAB scripts, numerical verification outputs, and figure-generation files used to reproduce all figures reported in the manuscript.
...
| threshold at ε=0.05 | **0.8755 (87.5%)** | Fig. 4 |
| r₁ at ε=0.01 | **6** (2730×) | Table A.1 |
...
All values above were independently cross-checked between the manuscript and the MATLAB scripts in this repository; see `MATLAB_OUTPUT_VALUES.txt` for the verification log.
...
| Result | Statement |
|:---|:---|
| **Theorem 4.1** (Occupation Measure) | μ_T = (1−ε)μ_s + εμ_f + O(ε²) in total variation |
...
- **MATLAB R2020a+** for figure generation
